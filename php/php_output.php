...
if($query_run=mysql_query($query1)){
					...
					echo"<tr><td colspan='2'>Total Bank Balance</td><td colspan='2' >$totalBalance</td></tr>";
...
function getQuantity($brand,$itemName,$itemPartNo){
	
	$query=" SELECT * FROM `inventory_core` WHERE `brand`='$brand' AND `item_name`='$itemName' AND `part_no`='$itemPartNo'";
	if($query_run=mysql_query($query)){
	if(mysql_num_rows($query_run)!=NULL){
		while($query_row=mysql_fetch_assoc($query_run))
				{
				$value=$query_row['quantity'];				
				echo $value;
				}
	}
	}
...
function getProvision($brand,$itemName,$itemPartNo){
...
function getUnitPrice($brand,$itemName,$itemPartNo){
...
function getPurchasePrice($brand,$itemName,$itemPartNo){
	...
	if(mysql_num_rows($query_run)!=NULL){
		while($query_row=mysql_fetch_assoc($query_run))
				{
				$value=$query_row['purchaseUnitPrice'];				
				echo $value;
				}
	}
	}
...
//echo $query1="SELECT * FROM `inventory_surface` ORDER BY id DESC LIMIT 20";
 $query1="SELECT * FROM `inventory_surface` ORDER BY id DESC ";
if($query_run=mysql_query($query1)){
//if(mysql_num_rows($query_run)==NULL){
$i=0;
	while($query_row=mysql_fetch_assoc($query_run)){					
						...
						echo "
								<tr>
								<td>$i</td><td>$date</td><td>$item_name</td><td>$part_no</td><td>$brand</td><td>$quantity</td><td class='inventoryDelete' >
<a href = 'javascript:void(0)' onclick = 'deletePopUp(\"DeleteConformation\",$id)'>Delete</a>
</td>
								</tr>
							";
...
$query1=" SELECT * FROM `client` WHERE `company` like '$name%' ";			
if($query_run=mysql_query($query1)){
		if(mysql_num_rows($query_run)==NULL){
				  echo "
					...
					";
						while($query_row=mysql_fetch_assoc($query_run))
							...
							<table>
								...
								<td class='noBorder'>
								<input type='button' value='Generate Challan' onclick=\"updateSurfaceInvoiceInfoAfterChallan($sl);pdfChallan()\"  class='submit'/>
								</td>
							 ...
							 $query1="SELECT * FROM `inventory_core` WHERE `part_no` LIKE '$part_no%' AND `item_name` LIKE '$item_name%' AND `brand` LIKE '$brand%'";
								if($query_run1=mysql_query($query1)){
									while($query_row1=mysql_fetch_assoc($query_run1))
											{	
												 $existQuantity=$query_row1['quantity'];
												 $existProvision=$query_row1['provision'];
												...
												//if($updateQuantity>0 && $updateProvision>0 ){
												 $query2="UPDATE `inventory_core` SET `quantity`='$updateQuantity',`provision`='$updateProvision' WHERE  `part_no` LIKE '$part_no%' AND `brand` LIKE '$brand%' AND `item_name` LIKE '$item_name%' ";
	...
	$totalPurchacePrice=0;
					while($query_row=mysql_fetch_assoc($query_run))
						{	 ++$sl;
						     ...
						     $saleUnitPrice=$query_row['saleUnitPrice'];
						     $purchaseUnitPrice=$query_row['purchaseUnitPrice'];
							...
							echo" <tr>
										...
										<td> $provision</td>										
										<td> $saleUnitPrice</td>										
										<td> $purchaseUnitPrice</td>										
									</tr>";
						$totalSellingPrice+=($quantity*$saleUnitPrice);
					    $totalPurchacePrice+=($quantity*$purchaseUnitPrice);
						}
						echo"<tr><td colspan='6'><b>value of stored product [ Total purchase price]</b></td><td colspan='2'>$totalPurchacePrice</td></tr>";
						echo"<tr><td colspan='6'><b>value of stored product [ Total sale price]</b></td><td colspan='2' >$totalSellingPrice</td></tr>";
						}
...
if($query_run=mysql_query($query)){
	...
	while($query_row=mysql_fetch_assoc($query_run)){
		...
		echo"<tr ><td colspan='8'>Total Unpaid Amount</td><td>$totalUnpaidIssue</td></tr>";
...
$totalWoingMoney=0;
	while($query_row=mysql_fetch_assoc($query_run)){
		...
		echo"<tr><td colspan='6'>Total Owing Amount </td><td>$totalWoingMoney</td></tr>";
...
if($query_run=mysql_query($query)){
	...
	while($query_row=mysql_fetch_assoc($query_run)){
		...
		//$due=$amount-$paid_amount;	
		$note=$query_row['note'];
			echo"<tr><td>$sl</td><td>$date</td><td>$voucher</td><td>$workOrder</td><td>$payTo</td><td>$mobile</td><td>$amount</td><td>$due</td><td>$note</td></tr>";
		}
		echo"<tr><td colspan='8'>Total Due Advance</td><td >$totalAdvance</td></tr>";
		}
	}
}
...
if($query_run=mysql_query($query)){
					if(mysql_num_rows($query_run)==NULL){
						...
						echo "
								<tr>
								<td>$sl</td><td>$workOrder</td><td>$voucher</td><td>$purpose</td><td>$pay_to</td><td>$mobile</td>
...
<a href = 'javascript:void(0)' onclick = 'deletePopUp(\"DeleteConformationDebitTransaction\",$voucher)'>Delete</a>
					...
					echo"<tr><td colspan='7'>Total Debit Amount </td><td>$totalDebitAmount</td></tr>";
...
if($query_run=mysql_query($query)){
					if(mysql_num_rows($query_run)==NULL){
						...
						echo "
								<tr>
								<td>$sl</td><td>$workOrder</td><td>$voucher</td><td>$purpose</td><td>$pay_from</td><td>$mobile</td>
								<td>$amount</td><td>$note</td>
								<!--<td class='inventoryDelete' >
<a href = 'javascript:void(0)' onclick = 'deletePopUp(\"DeleteConformationCreditTransaction\",$voucher)'>Delete</a>
						...
						echo"<tr><td colspan='7'>Total Credit Amount </td><td>$totalCreditAmount</td></tr>";
...
if($query_run=mysql_query($query)){
    while($query_row=mysql_fetch_assoc($query_run)){
			 ...
			 return $grandTotal=$subtotal+$vat+$tax-$discount;
}
}
?>